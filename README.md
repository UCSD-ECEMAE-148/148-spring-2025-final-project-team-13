...
**struggles on having the stop sign model running on the camera**
When starting to train the roboflow model, we decided to take our one data set with the camera so that the accuracy would be better, however, due to the lack of physical stop sign, we can only take pictures of printed stop sign pictures and pictures online, and it took us a lot of time finding pictures for both true and fake stop signs and taking pictures with the OAK camera. Fortunately, thanks for the physical stop sign provided by Alex, it was much easier for collecting data and testing the model trained online. What's more, while testing the model, we found that the catagory which was fake stop sign sometimes affected the accuracy of detecting the true stop sign, so we decided to retrain the model for stop sign detection instead of classification. After having the Roboflow model trained online, we found that the latency was high since the model needed to be loaded from the Roboflow API, so we decided to retrain the model locally and have it run directly on the camera, and it was a hard time loading the labelled dataset, retraining the model locally and convert the trained model into the .blob file which could be used by the camera directly. Due to the final success of stop sign model, we also helped other team who are also struggling with running the model locally.
